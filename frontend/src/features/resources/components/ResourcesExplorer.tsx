...
export function ResourcesExplorer({ resources }: { resources: PublicResource[] }) {
  ...
  const filtered = useMemo(() => {
    const keyword = appliedQuery.trim().toLocaleLowerCase("vi");
      ...
      <section className={styles.explorer} aria-labelledby="resource-heading">
        <header className={styles.intro}>
          <div>
            ...
            <h2 id="resource-heading">Tra cứu tài nguyên</h2>
          </div>
          <span>Tài liệu được cập nhật trực tiếp từ hệ thống quản trị của Viện Sau Đại học.</span>
        </header>
        <form className={styles.filters} onSubmit={search}>
          <label className={styles.search}>
            ...
            <input
              ...
              type="search"
              placeholder="Tìm theo tên, số văn bản hoặc đơn vị ban hành..."
            />
          </label>
        ...
        <div className={styles.resultBar}>
          <b>{filtered.length} tài liệu</b>
          <span>
            {appliedQuery || category !== "ALL" || type !== "ALL"
            ...
            {filtered.map((item) => (
              <article className={styles.card} key={item.id}>
                <div className={styles.fileIcon}>
                  <FileText />
                  <span>{item.fileType.split("/").pop()?.toUpperCase() || "FILE"}</span>
                </div>
                <div className={styles.cardBody}>
                  <div className={styles.badges}>
                    <span>{item.category.name}</span>
                    <i>{resourceTypeLabels[item.resourceType] ?? item.resourceType}</i>
                    {item.documentType && (
                      <i>{documentTypeLabels[item.documentType] ?? item.documentType}</i>
                    )}
                  </div>
                  <h3>{item.title}</h3>
                  ...
                  <dl>
                    ...
                    {item.issuingOrganization && (
                      <div>
                        ...
                        <dd>{item.issuingOrganization}</dd>
                      </div>
                    )}
                    <div>
                      <dt>Tệp</dt>
      ...
      {preview && (
        ...
        >
          <section
            ...
            role="dialog"
            aria-modal="true"
            aria-label={`Xem trước ${preview.title}`}
          ...
          >
            <header>
              ...
              <button onClick={() => setPreview(null)} aria-label="Đóng">
                <X />
              </button>
            </header>
            <div className={styles.previewFrame}>
              {preview.fileType.includes("pdf") || preview.fileType.startsWith("image/") ? (
                <iframe src={fileEndpoint(preview)} title={preview.title} />
              ) : (
                <div>
                  <FileText />
                  <p>Trình duyệt không hỗ trợ xem trước định dạng này.</p>
                  <a href={fileEndpoint(preview, true)}>Mở hoặc tải tài liệu</a>
                </div>
              )}
            </div>
            <footer>
              ...
              <a href={fileEndpoint(preview, true)}>
                <Download />
                Tải xuống
              </a>
            </footer>
