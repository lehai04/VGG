...
| **Lưu Trữ Tệp khi Mở Rộng Ngang (Multi-server Scaling)** | `LOW` | Mở rộng nhiều máy chủ ứng dụng | MinIO tách biệt khỏi tiến trình web nên các instance dùng chung một bucket. Khi mở rộng, IT cần triển khai MinIO distributed hoặc cụm S3-compatible nội bộ và giữ nguyên cấu hình `STORAGE_*`. |
